...
const JobDescriptionPage = () => {
  ...
  const applyForJob = async () => {
    ...
    try {
      const res = await axios.post(`lucky-job.vercel.app/api/v1/application/applyjob/${jobId}`,  {},{
  ...
  useEffect(() => {
    const fetchSingleJobs = async () => {
      try {
        const res = await axios.get(`lucky-job.vercel.app/api/v1/job/getjob/${jobId}`, { withCredentials: true })
        if (res.data.success) {
